...
let productName = "Homeworks"
let sln = __SOURCE_DIRECTORY__ </> ".." </> "Homeworks.sln"
...
let dotnetRestore _ =
    ...
    |> Seq.map (fun dir ->
        fun () ->
            let args = []
            DotNet.restore (fun c -> { c with Common = c.Common |> DotNet.Options.withAdditionalArgs args }) dir)
