...
type Graph<'Value when 'Value: equality> =
    ...
    val Vertices: uint
    val Edges: uint
    new(memory, vertices, edges) = {Memory = memory; Vertices = vertices; Edges = edges}
...
type ReadFile<'Value when 'Value: equality>=
    struct
        ...
        new (numData, mType, vertices, entries, sparseMatrix) = {NumericData = numData; MatrixType = mType; Vertices = vertices; Entries = entries; SparseMatrix = sparseMatrix}
    end
